 ...
 **dotnet run** - запустить проэкт
 ...
 __ReadKey()__ - нажать любую клавишу
...
__This.__ - команда,для 1.резрешения неоднозного присваимания,когда имена параметров в классе аналогичны именам передаваемым переменным.
    ...
    В таком случае,передеаваемая переменная name как аргумент в конструкторе,присвоится параметру name,который находится вне конструра
                        
   2.Для передачи другим методам объекта в качестве параметра
      
      Designer(string name,int age):this(name)
      {
       this.age = age
      }
      Тут значения передаваемой переменной переменной name будут унаследованны,из предыдущего конструктора у параметра name
      
    
   Получить доступ к текущему экземлпляру класса
Используется для решения,какой-то неоднозначности в названиях переменных


### Наименования хорошего тона

    !Переменные,методы следует наименовывать так,чтобы было понятно за что это отвечает не углубляясь в код!
    Методы - NameNameName
    Класс - NameName(сущ.)
    Константы - NAME
    Приватыне поля(privare) - _nameName
    Публичные поля(public) - NameName
